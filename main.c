...
typedef struct Node {//Node를 struct로 선언
	...
	struct Node* llink; 
	struct Node* rlink;
} listNode; //구조체 별명을 listNode로 선언

/* 함수 리스트 */
int initialize(listNode** h); //headNode에 메모리를 할당하는 함수 선언
int freeList(listNode* h); //할당된 메모리 해제하는 함수 선언
int insertLast(listNode* h, int key); //list의 끝에 key에 대한 노드를 추가하는 함수 선언
int deleteLast(listNode* h); //list의 마지막 노드를 삭제하는 함수 선언
int insertFirst(listNode* h, int key); //list의 처음에 key에 대한 노드를 추가하는 함수 선언
int deleteFirst(listNode* h); //list의 처음 노드를 삭제하는 함수 선언
int invertList(listNode* h); //list의 link를 역순으로 다시 배열하는 함수 선언
...
int main()
{
	char command; 
	int key;
	listNode* headnode=NULL;  //listNode 에 대한 포인터 headnode 
...
int initialize(listNode** h) {
	...
	if(*h != NULL) //포인터 h가 NULL이 아닐때 /headNode가 NULL이 아닐 때 
		freeList(*h); // 메모리 해제
...
int freeList(listNode* h){ // h와 연결된 listNode 메모리 해제
	...
	while(p != NULL && p != h) { //p가 NULL이 아니고 h도 아닐 때 반복
		prev = p; //prev 에 p 저장
...
void printList(listNode* h) { 
	...
	while(p != NULL && p != h) { //p가 NULL이 아니고 p가 h도 아닐 때 반복
...
int deleteNode(listNode* h, int key) {
	...
	while(n != NULL && n != h) { //n이 NULL이 아니고 h도 아닐 때 반복
